    ...
    def CrearVentana(self):
        ...
        while True:
            ...
            if enJuego == True:
                ...
                # condicion si no se presionan teclas
                elif event.type == pygame.KEYUP:
                    ventana.blit(jugador.imagenCar, jugador.rectCar)
                    jugador.numero = 0
                    velx = 0
                    vely = 0
                    # condicion si no se presiona la tecla arriba
                    if event.key == K_UP:
                        jugador.sonidoFrenado.play(1)
                        jugador.sonidoAcel.stop()
                        jugador.aceleracion = 0
                    # condicion si no se presiona la tecla abajo
                    elif event.key == K_DOWN:
                        jugador.sonidoFrenado.stop()
                        jugador.sonidoAcel.stop()
                    # condicion si no se presiona la tecla izquierda
                    elif event.key == K_LEFT:
                        jugador.sonidoFrenado.play(1)
                        jugador.sonidoAcel.stop()
                        jugador.aceleracion = 0
                    # condicion si no se presiona la tecla derecha
                    elif event.key == K_RIGHT:
                        jugador.sonidoFrenado.play(1)
                        jugador.sonidoAcel.stop()
                        jugador.aceleracion = 0
